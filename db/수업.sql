...
select bookname '제목', char_length(bookname)'문자 수' , length(bookname) from book where publisher = '굿스포츠';
...
select name,bookname from customer,book,orders where customer.custid = orders.custid and orders.bookid = book.bookid; 

select name,bookname from book,orders,customer where price = 20000 and customer.custid = orders.custid and book.bookid = orders.bookid;

select name,saleprice from orders right outer join customer on customer.custid = orders.custid;

select bookname from book where price = (select max(price) from book );

select name from customer where custid in (select custid from orders where bookid in(select bookid from book where publisher = '대한출판사'));
...
update book set publisher = (select publisher from imported_book where bookid = 21) where bookid = 7;
select * from book;

select distinct name from customer,book,orders where customer.custid = orders.custid and book.bookid = orders.bookid and name not in ('박지성')
and book.publisher in (select distinct book.publisher from book,orders where book.bookid = orders.bookid );
...
-- 4-4) 도서 제목에 야구가 포함된 도서를 농구로 변경한 수 도서목록을 보이시오.
select bookid, replace(bookname,'야구','농구') bookname,publisher,price from book;

-- 4-5) 굿스포츠에서 출간한 도서의 제목과 제목의 문자수,바이트 수를 보이시오.
select bookname '제목', char_length(bookname)'문자 수' , length(bookname) from book where publisher = '굿스포츠';

 -- 4-6) 마당서점의 고객 중에서 같은 성을 가진 사람이 몇 명이나 되는지 성별 인원수를 구하시오.
select substr(name,1,1) '성', count(*)'인원' from customer group by substr(name,1,1);

 -- 4-7) 마당 서점은 주문일로부터 10일 후 매출을 확정한다. 각 주문의 확정일자를 구하시오.
select orderid '주문번호' , orderdate '주문일' , adddate(orderdate,interval 10 day)'확정' from orders;

-- 4-8) 마당서점이 2014.07.07에 주문받은 도서를 모두 보이시오.
select orderid '주문번호' , str_to_date(orderdate,'%Y-%m-%d') '주문일', custid '고객번호', bookid '도서번호' from orders where orderdate = date_format('20140707','%Y-%m-%d'); 

-- 4-9) DBMS 서버에 현재 설정된 날짜와 시간 요일 확인하시오.
select sysdate(), date_format(sysdate(),'%Y/%m/%d %M %h:%s') 'sysdate_1'; 
